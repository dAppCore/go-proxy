...
func TestAccesslogImpl_LogSink_Close_Ugly(t *testing.T) {
	sink := newAccessLogSink(filepath.Join(t.TempDir(), "access.log"))
	sink.Close()
	sink.Close()
	if sink.file != nil {
...
func TestAccesslogImpl_LogSink_OnLogin_Good(t *testing.T) {
	...
	sink.Close()
	data, err := os.ReadFile(path)
	if err != nil || !strings.Contains(string(data), "CONNECT") {
...
func TestAccesslogImpl_LogSink_OnClose_Good(t *testing.T) {
	...
	sink.Close()
	data, err := os.ReadFile(path)
	if err != nil || !strings.Contains(string(data), "CLOSE") {
...
func TestAccesslogImpl_LogSink_OnClose_Ugly(t *testing.T) {
	...
	sink.Close()
	data, err := os.ReadFile(path)
	if err != nil || !strings.Contains(string(data), "rx=18446744073709551615") {
