...
func stateImplJob() Job {
	return Job{JobID: "job-1", Blob: strings.Repeat("0", 160), Target: "b88d0600", Algo: "rx/0"}
}
...
func TestStateImpl_Proxy_ServerListenerAddr_Good(t *testing.T) {
	...
	if got := p.ServerListenerAddr(0); !strings.Contains(got, "127.0.0.1:") {
...
func TestStateImpl_Miner_CurrentJob_Ugly(t *testing.T) {
	miner := &Miner{currentJob: Job{JobID: "edge", Blob: strings.Repeat("f", 160), Target: "ffffffff"}}
...
func TestStateImpl_Miner_ForwardJob_Good(t *testing.T) {
	...
	if len(conn.writes) != 1 || !strings.Contains(conn.writes[0], `"method":"job"`) {
...
func TestStateImpl_Miner_ReplyWithError_Good(t *testing.T) {
	...
	if len(conn.writes) != 1 || !strings.Contains(conn.writes[0], "Invalid nonce") {
...
func TestStateImpl_Miner_ReplyWithError_Ugly(t *testing.T) {
	...
	if len(conn.writes) != 1 || !strings.Contains(conn.writes[0], `"message":""`) {
...
func TestStateImpl_Miner_Success_Good(t *testing.T) {
	...
	miner := &Miner{conn: conn}
	miner.Success(3, "OK")
	if len(conn.writes) != 1 || !strings.Contains(conn.writes[0], `"status":"OK"`) {
...
func TestStateImpl_Miner_Success_Ugly(t *testing.T) {
	...
	miner := &Miner{conn: conn}
	miner.Success(0, "")
	if len(conn.writes) != 1 || !strings.Contains(conn.writes[0], `"status":""`) {
