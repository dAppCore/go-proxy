...
//
//	job := proxy.Job{Blob: repeatString("0", 160)}
//	result := job.BlobWithFixedByte(0x2A) // chars 78-79 become "2a"
func TestJob_Job_BlobWithFixedByte_Good(t *testing.T) {
	job := Job{Blob: repeatString("0", 160), JobID: "job-1", Target: "b88d0600"}
...
func TestJob_Job_BlobWithFixedByte_Bad(t *testing.T) {
	shortBlob := "0000"
...
func TestJob_Job_BlobWithFixedByte_Ugly(t *testing.T) {
	job := Job{Blob: repeatString("0", 160), JobID: "job-1", Target: "b88d0600"}
...
//	job.IsValid() // true
func TestJob_Job_IsValid_Good(t *testing.T) {
	job := Job{Blob: repeatString("0", 160), JobID: "job-1", Target: "b88d0600"}
	if !job.IsValid() {
...
//	job.IsValid() // false
func TestJob_Job_IsValid_Bad(t *testing.T) {
	if (Job{Blob: "", JobID: "job-1", Target: "b88d0600"}).IsValid() {
		t.Fatalf("expected empty blob to be invalid")
	}
	if (Job{Blob: repeatString("0", 160), JobID: "", Target: "b88d0600"}).IsValid() {
		t.Fatalf("expected empty job id to be invalid")
	}
	if (Job{Blob: repeatString("0", 160), JobID: "job-1", Target: "bad"}).IsValid() {
...
//	job := proxy.Job{}
//	job.IsValid() // false
func TestJob_Job_IsValid_Ugly(t *testing.T) {
	if (Job{}).IsValid() {
