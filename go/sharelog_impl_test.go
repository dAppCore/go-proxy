...
func TestSharelogImpl_LogSink_Close_Ugly(t *testing.T) {
	sink := newShareLogSink(pathJoin(t.TempDir(), "shares.log"))
	sink.Close()
	sink.Close()
	if sink.file != nil {
...
func TestSharelogImpl_LogSink_OnAccept_Good(t *testing.T) {
	...
	sink.Close()
	data, err := readFile(path)
	if err != nil || !containsString(string(data), "ACCEPT  wallet  diff=12  latency=3ms") {
...
func TestSharelogImpl_LogSink_OnReject_Good(t *testing.T) {
	...
	sink.Close()
	data, err := readFile(path)
	if err != nil || !containsString(string(data), "REJECT  wallet") {
...
func TestSharelogImpl_LogSink_OnReject_Ugly(t *testing.T) {
	...
	sink.Close()
	data, err := readFile(path)
	if err != nil || !containsString(string(data), "bad\\\" reason") {
