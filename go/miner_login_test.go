...
func (failingRandReader) Read([]byte) (int, error) {
	return 0, core.NewError("forced rand failure")
}
...
func (c *loginRejectConn) Write(data []byte) (int, error) {
	c.data = append(c.data, data...)
	return len(data), nil
}
func (c *loginRejectConn) Bytes() []byte                    { return append([]byte(nil), c.data...) }
...
func TestMiner_HandleLogin_Good(t *testing.T) {
	...
	params, err := testJSONMarshal(loginParams{
		Login: "wallet",
		Pass:  "x",
		Agent: "xmrig",
		Algo:  []string{"cn/r"},
...
func TestMiner_HandleLogin_Bad(t *testing.T) {
	...
	t.Run("uuid_generation_failure", func(t *testing.T) {
		...
		if err := testJSONUnmarshal(conn.Bytes(), &payload); err != nil {
			t.Fatalf("unmarshal login rejection: %v", err)
		}
...
func TestMiner_HandleLogin_AccessPassword_Good(t *testing.T) {
	// target tokens: HandleLogin AccessPassword
	// target symbol: HandleLogin_AccessPassword
	minerConn, clientConn := net.Pipe()
	defer minerConn.Close()
	defer clientConn.Close()
...
func TestMiner_HandleLogin_CustomDiffCap_Good(t *testing.T) {
	// target tokens: HandleLogin CustomDiffCap
	// target symbol: HandleLogin_CustomDiffCap
	minerConn, clientConn := net.Pipe()
	defer minerConn.Close()
	defer clientConn.Close()
...
func TestMiner_HandleLogin_CustomDiffSuffix_Good(t *testing.T) {
	// target tokens: HandleLogin CustomDiffSuffix
	// target symbol: HandleLogin_CustomDiffSuffix
	minerConn, clientConn := net.Pipe()
	defer minerConn.Close()
	defer clientConn.Close()
...
func TestMiner_HandleKeepalived_Good(t *testing.T) {
	// target symbol: HandleKeepalived
	minerConn, clientConn := net.Pipe()
	defer minerConn.Close()
	defer clientConn.Close()
	...
	var payload map[string]testRawMessage
	if err := testJSONUnmarshal(line, &payload); err != nil {
	...
	if err := testJSONUnmarshal(payload["result"], &result); err != nil {
...
func TestMiner_ReadLoop_RFCLineLimit_Good(t *testing.T) {
	// target tokens: ReadLoop RFCLineLimit
	// target symbol: ReadLoop_RFCLineLimit
	minerConn, clientConn := net.Pipe()
	defer minerConn.Close()
	defer clientConn.Close()
	...
	params, err := testJSONMarshal(loginParams{
		Login: "wallet",
		Pass:  "x",
		Agent: repeatString("a", 5000),
	})
	if err != nil {
		t.Fatalf("marshal login params: %v", err)
	}
	request, err := testJSONMarshal(stratumRequest{ID: 4, Method: "login", Params: params})
	if err != nil {
...
func TestMiner_ReadLoop_RFCLineLimit_Ugly(t *testing.T) {
	...
	params, err := testJSONMarshal(loginParams{
		Login: "wallet",
		Pass:  "x",
		Agent: repeatString("b", maxStratumLineLength),
	})
	if err != nil {
		t.Fatalf("marshal login params: %v", err)
	}
	request, err := testJSONMarshal(stratumRequest{ID: 5, Method: "login", Params: params})
	if err != nil {
		t.Fatalf("marshal request: %v", err)
	}
	if len(request) <= maxStratumLineLength {
	...
	if !containsString(writeErr.Error(), "closed pipe") {
