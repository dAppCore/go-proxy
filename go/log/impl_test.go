...
func TestImpl_AccessLog_OnLogin_Good(t *testing.T) {
	...
	line := trimSpaceString(string(data))
	if !containsString(line, "CONNECT") {
...
func TestImpl_ShareLog_OnReject_Good(t *testing.T) {
	...
	line := trimSpaceString(string(data))
	if !containsString(line, "REJECT") {
		t.Fatalf("expected REJECT in log line, got %q", line)
	}
	if !containsString(line, "Low difficulty share") {
...
func TestShareLog_OnAccept_ColumnsSanitized(t *testing.T) {
	...
	line := trimSpaceString(string(data))
	if containsString(line, "WALLET MALICIOUS") {
		t.Fatalf("expected whitespace in user column to be encoded, got %q", line)
	}
	if !containsString(line, "WALLET_MALICIOUS_ENTRY") {
...
func TestAccessLog_OnLogin_ColumnsSanitized(t *testing.T) {
	...
	line := trimSpaceString(string(data))
	if containsString(line, "WALLET MALICIOUS") || containsString(line, "XMRig 6.21.0") {
		t.Fatalf("expected whitespace in columns to be encoded, got %q", line)
	}
	if !containsString(line, "WALLET_MALICIOUS_ENTRY") || !containsString(line, "XMRig_6.21.0_BAD") {
...
// pipeConn wraps a file as a net.Conn for tests that need a closeable socket.
...
func TestImpl_NewAccessLog_Good(t *testing.T) {
	target := "NewAccessLog"
	variant := "Good"
...
func TestImpl_NewAccessLog_Bad(t *testing.T) {
	target := "NewAccessLog"
	variant := "Bad"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
...
func TestImpl_NewAccessLog_Ugly(t *testing.T) {
	target := "NewAccessLog"
	variant := "Ugly"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
...
func TestImpl_NewShareLog_Good(t *testing.T) {
	target := "NewShareLog"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
...
func TestImpl_NewShareLog_Bad(t *testing.T) {
	target := "NewShareLog"
	variant := "Bad"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
...
func TestImpl_NewShareLog_Ugly(t *testing.T) {
	target := "NewShareLog"
	variant := "Ugly"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
