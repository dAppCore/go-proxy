...
func startProxyInstance(t *testing.T, config *proxy.Config) *proxy.Proxy {
	...
	go instance.Start()
	waitFor(t, 2*time.Second, func() bool {
		conn, err := net.DialTimeout("tcp", net.JoinHostPort("127.0.0.1", core.Sprintf("%d", config.Bind[0].Port)), 100*time.Millisecond)
		if err != nil {
			return false
		}
		_ = conn.Close()
...
func handleNicehashPoolConnection(conn net.Conn) {
	...
	for {
		...
		if err := testJSONUnmarshal(line, &request); err != nil {
			return
		}
...
func loginMiner(t *testing.T, address, login string) (net.Conn, *bufio.Reader, minerLoginResponse) {
	...
	if err := testJSONUnmarshal(line, &response); err != nil {
	...
	if response.Error == nil && (response.Result == nil || response.Result.Job.JobID == "") {
		...
		for time.Now().Before(deadline) {
			...
			if err := testJSONUnmarshal(line, &notification); err != nil {
				continue
			}
...
func encodeLineBytes(payload any) []byte {
	data, _ := testJSONMarshal(payload)
	return append(data, '\n')
}

func zeroBlob() string {
	return repeatString("0", 160)
}
