...
func TestSimpleMapperFile_OnJob_ResetsPreviousJob_Good(t *testing.T) {
	mapper := NewSimpleMapper(7, activeStrategy{})
	mapper.currentJob = proxy.Job{Blob: repeatString("0", 160), JobID: "job-1", Target: "b88d0600", ClientID: "session-1"}
	mapper.OnJob(proxy.Job{Blob: repeatString("1", 160), JobID: "job-2", Target: "b88d0600", ClientID: "session-1"})
	if mapper.prevJob.JobID != "job-1" {
		t.Fatalf("expected previous job to be retained, got %q", mapper.prevJob.JobID)
	}
}

func TestMapper_NewSimpleMapper_Good(t *testing.T) {
	target := "NewSimpleMapper"
	variant := "Good"
...
func TestMapper_NewSimpleMapper_Bad(t *testing.T) {
	target := "NewSimpleMapper"
	variant := "Bad"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
...
func TestMapper_NewSimpleMapper_Ugly(t *testing.T) {
	target := "NewSimpleMapper"
	variant := "Ugly"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
