...
func TestMapper_Submit_InvalidJob_Good(t *testing.T) {
	...
	strategy := &startCountingStrategy{}
	mapper := NewNonceMapper(1, &proxy.Config{}, strategy)
	mapper.storage.job = proxy.Job{JobID: "job-1", Blob: repeatString("0", 160), Target: "b88d0600"}
...
func TestMapper_OnResultAccepted_ExpiredUsesPreviousJob(t *testing.T) {
	...
	mapper.events = bus
	mapper.storage.job = proxy.Job{JobID: "job-new", Blob: repeatString("0", 160), Target: "b88d0600"}
	mapper.storage.prevJob = proxy.Job{JobID: "job-old", Blob: repeatString("0", 160), Target: "b88d0600"}
...
func TestMapper_Submit_ExpiredJobUsesPreviousDifficulty(t *testing.T) {
	...
	mapper := NewNonceMapper(1, &proxy.Config{}, strategy)
	mapper.storage.job = proxy.Job{JobID: "job-new", Blob: repeatString("0", 160), Target: "ffffffff"}
	mapper.storage.prevJob = proxy.Job{JobID: "job-old", Blob: repeatString("0", 160), Target: "b88d0600"}
	mapper.storage.miners[miner.ID()] = miner
