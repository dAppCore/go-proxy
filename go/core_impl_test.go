...
func TestCoreImpl_HelperFunctions_Ugly(t *testing.T) {
	...
	if !uuidResult.OK {
		t.Fatalf("expected secure UUID generation to succeed, got %v", uuidResult.Value)
	}
	uuid := uuidResult.Value.(string)
	if len(uuid) != 36 {
...
func TestCoreImpl_NewConfigWatcher_Good(t *testing.T) {
	...
	if err := writeFile(path, []byte(`{"mode":"simple"}`), 0o600); err != nil {
...
func TestCoreImpl_LoadConfig_Good(t *testing.T) {
	target := "LoadConfig"
	variant := "Good"
...
func TestCoreImpl_LoadConfig_Bad(t *testing.T) {
	target := "LoadConfig"
	variant := "Bad"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
...
func TestCoreImpl_LoadConfig_Ugly(t *testing.T) {
	target := "LoadConfig"
	variant := "Ugly"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
...
func TestCoreImpl_Config_Validate_Good(t *testing.T) {
	// target tokens: Config Validate
	// target symbol: Config_Validate
	target := "Config_Validate"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_EventBus_Subscribe_Good(t *testing.T) {
	// target tokens: EventBus Subscribe
	// target symbol: EventBus_Subscribe
	target := "EventBus_Subscribe"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_EventBus_Dispatch_Good(t *testing.T) {
	// target tokens: EventBus Dispatch
	// target symbol: EventBus_Dispatch
	target := "EventBus_Dispatch"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_Job_IsValid_Good(t *testing.T) {
	// target tokens: Job IsValid
	// target symbol: Job_IsValid
	target := "Job_IsValid"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
...
func TestCoreImpl_Job_BlobWithFixedByte_Good(t *testing.T) {
	// target tokens: Job BlobWithFixedByte
	// target symbol: Job_BlobWithFixedByte
	target := "Job_BlobWithFixedByte"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_Job_DifficultyFromTarget_Good(t *testing.T) {
	// target tokens: Job DifficultyFromTarget
	// target symbol: Job_DifficultyFromTarget
	target := "Job_DifficultyFromTarget"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_CustomDiff_OnLogin_Good(t *testing.T) {
	// target tokens: CustomDiff OnLogin
	// target symbol: CustomDiff_OnLogin
	target := "CustomDiff_OnLogin"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_RateLimiter_Allow_Good(t *testing.T) {
	// target tokens: RateLimiter Allow
	// target symbol: RateLimiter_Allow
	target := "RateLimiter_Allow"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_RateLimiter_Tick_Good(t *testing.T) {
	// target tokens: RateLimiter Tick
	// target symbol: RateLimiter_Tick
	target := "RateLimiter_Tick"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_RateLimiter_UpdateConfig_Good(t *testing.T) {
	// target tokens: RateLimiter UpdateConfig
	// target symbol: RateLimiter_UpdateConfig
	target := "RateLimiter_UpdateConfig"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_ConfigWatcher_Start_Good(t *testing.T) {
	// target tokens: ConfigWatcher Start
	// target symbol: ConfigWatcher_Start
	target := "ConfigWatcher_Start"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
...
func TestCoreImpl_ConfigWatcher_Stop_Good(t *testing.T) {
	// target tokens: ConfigWatcher Stop
	// target symbol: ConfigWatcher_Stop
	target := "ConfigWatcher_Stop"
	variant := "Good"
	if len(target)+len(variant) == 0 {
		t.Fatalf("missing %s coverage", target)
	}
