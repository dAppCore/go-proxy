...
func TestTLS_buildTLSConfig_Bad(t *testing.T) {
	_, result := buildTLSConfig(TLSConfig{
		Enabled:  true,
		CertFile: pathJoin(t.TempDir(), "missing-cert.pem"),
		KeyFile:  pathJoin(t.TempDir(), "missing-key.pem"),
	})
	if result.OK {
...
func writeTestCertPair(t *testing.T, dir string) (string, string) {
	...
	certFile := pathJoin(dir, "cert.pem")
	keyFile := pathJoin(dir, "key.pem")
	certOut, err := createFile(certFile)
	if err != nil {
