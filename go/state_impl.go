...
func (m *Miner) handleLogin(request stratumRequest) {
	...
	rpcIDResult := generateUUID()
	if !rpcIDResult.OK || rpcIDResult.Value.(string) == "" {
		m.mu.Unlock()
		m.rejectLogin(requestID(request.ID), "Proxy is unavailable, try again later")
		return
	}
	rpcID := rpcIDResult.Value.(string)
	m.rpcID = rpcID
...
func (m *Miner) replyLoginSuccess(id int64) {
	...
	if r := m.writeJSONLocked(payload); !r.OK {
		return
	}
}
...
func (m *Miner) writeJSON(payload any) core.Result {
	m.sendMu.Lock()
	defer m.sendMu.Unlock()
	return m.writeJSONLocked(payload)
}

func (m *Miner) writeJSONLocked(payload any) core.Result {
	m.mu.RLock()
	conn := m.conn
	m.mu.RUnlock()
	if conn == nil {
